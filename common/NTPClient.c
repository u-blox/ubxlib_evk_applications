...
int64_t getNTPTime(void)
{
    ...
    *( ( char * ) &packet + 0 ) = 0x1b;
    ...
    uSockDescriptor_t sock = uSockCreate(gCellDeviceHandle, U_SOCK_TYPE_DGRAM, U_SOCK_PROTOCOL_UDP);
    if(sock < 0) {
        writeInfo("Failed to create UDP socket: %d", sock);
        return sock;
    }
    ...
    int32_t errorCode = uSockGetHostByName(gCellDeviceHandle, NTP_SERVER, &(remoteAddress.ipAddress));
    if (errorCode == 0) {
        ...
        if (errorCode > 0) {
            if (waitForSocketData()) {
                ...
                if (errorCode > 0) {
                    writeInfo("Received NTP time from server");
